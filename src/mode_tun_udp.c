                ...
                /* ── Data frame from known client ── */
                {
                    ...
                    if (frame_parse_explicit(buf, (size_t)nr, &ty, &fl,
                        ...
                        if (ty == FRAME_DATA && pl > 0) {
                            ...
                            /*
                             * Drain any TUN replies generated by this write.
                             * WireGuard processes the reply in the same context
                             * instead of waiting for the next poll() cycle.
                             */
                            for (;;) {
                                uint8_t reply[FRAME_MAX_PAYLOAD];
                                ssize_t pn = read(tun_fd, reply, sizeof(reply));
                                if (pn < 0) {
                                    if (errno == EAGAIN || errno == EWOULDBLOCK) break;
                                ...
                                log_info("udp-server", "TUN reply: %zd bytes, ver=%d", pn, (reply[0] >> 4));

                                /* Route and send immediately */
                                uint32_t rdst = ip_dst_addr(reply, (size_t)pn);
                                int rsent = 0;
                                for (int rsi = 0; rsi < session_count; rsi++) {
                                    if (!sessions[rsi].active) continue;
                                    if (rdst != 0 && sessions[rsi].tun_ip != 0 &&
                                        rdst != sessions[rsi].tun_ip) continue;

                                    uint8_t wb[FRAME_EXPLICIT_MAX_WIRE]; size_t wl;
                                    if (frame_build_explicit(wb, &wl, FRAME_DATA, 0,
                                                              reply, (size_t)pn,
                                                              sessions[rsi].enc_key) == 0) {
                                        ssize_t sn = sendto(udp_fd, wb, wl, 0,
                                               (struct sockaddr *)&sessions[rsi].addr,
                                               sizeof(sessions[rsi].addr));
                                        log_info("udp-server", "  → sent %zd to %s:%d",
                                                 sn,
                                                 inet_ntop(AF_INET, &sessions[rsi].addr.sin_addr,
                                                           (char[INET_ADDRSTRLEN]){}, INET_ADDRSTRLEN),
                                                 ntohs(sessions[rsi].addr.sin_port));
                                        rsent++;
                                    }
                                }
                                if (rsent == 0)
                                    log_info("udp-server", "TUN reply dst=0x%x unmatched (%d active)",
                                             rdst, session_count);
                            }
                        }
                    }
