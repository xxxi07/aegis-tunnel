...
static void H(uint8_t out[32], const uint8_t *in, size_t len) {
...
static void set_to(int fd, int ms) { if(ms<=0)return; struct timeval tv={.tv_sec=ms/1000,.tv_usec=(ms%1000)*1000}; setsockopt(fd,SOL_SOCKET,SO_RCVTIMEO,&tv,sizeof(tv)); }
...
static int psk_init_key(uint8_t ik[16], const uint8_t *psk, size_t plen) {
    ...
    memcpy(b,psk,plen); memcpy(b+plen,PSK_DOMAIN,dl); uint8_t h[32]; H(h,b,bl); free(b);
...
int handshake_server(int fd, const uint8_t *psk, size_t plen, int timeout_ms, session_keys_t *keys) {
    ...
    set_to(fd,timeout_ms); if(plen<16||psk_init_key(ik,psk,plen)!=0)return -1;
    if(psk_recv(fd,cn,&cts,cpub,ik)!=0){secure_memzero(ik,16);return -1;}
    if(check_ts(cts)!=0){secure_memzero(ik,16);return -1;}
    if(ecdh_keygen(epub,epriv)!=0||random_bytes(sn,16)!=0){secure_memzero(ik,16);return -1;}
    if(ecdh_derive(sh,epriv,cpub)!=0){secure_memzero(epriv,32);secure_memzero(ik,16);return -1;}
    secure_memzero(epriv,32);
    if(psk_session(keys,psk,plen,sh,cn,sn)!=0){secure_memzero(sh,32);secure_memzero(ik,16);return -1;}
...
int handshake_client(int fd, const uint8_t *psk, size_t plen, int timeout_ms, session_keys_t *keys) {
    ...
    set_to(fd,timeout_ms); if(plen<16||psk_init_key(ik,psk,plen)!=0)return -1;
    if(ecdh_keygen(epub,epriv)!=0||random_bytes(cn,16)!=0){secure_memzero(ik,16);return -1;}
    int64_t cts=timestamp_now();
    if(psk_send(fd,cn,cts,epub,ik)!=0){secure_memzero(epriv,32);secure_memzero(ik,16);return -1;}
    if(psk_recv(fd,sn,&sts,spub,ik)!=0){secure_memzero(epriv,32);secure_memzero(ik,16);return -1;}
    if(check_ts(sts)!=0){secure_memzero(epriv,32);secure_memzero(ik,16);return -1;}
    if(ecdh_derive(sh,epriv,spub)!=0){secure_memzero(epriv,32);secure_memzero(ik,16);return -1;}
    secure_memzero(epriv,32);
    if(psk_session(keys,psk,plen,sh,cn,sn)!=0){secure_memzero(sh,32);secure_memzero(ik,16);return -1;}
    secure_memzero(sh,32);secure_memzero(cn,16);secure_memzero(sn,16);
...
int handshake_key_confirm_server(int fd, const session_keys_t *k, int to) {
    set_to(fd,to); uint8_t wb[FRAME_MAX_WIRE]; size_t wl=0;
...
int handshake_key_confirm_client(int fd, const session_keys_t *k, int to) {
    set_to(fd,to); uint8_t wb[FRAME_HEADER_LEN+AEGIS_TAG_LEN];
...
int handshake_rekey(int fd, const uint8_t *psk, size_t plen, session_keys_t *keys, uint64_t *nc, int init) {
    ...
    if(init){
        if(frame_build(wb,&wl,FRAME_REKEY,FLAG_NONE,opub,32,*nc,keys->enc_key)!=0){secure_memzero(opriv,32);return -1;}
        (*nc)++;if(send_all(fd,wb,wl)!=0){secure_memzero(opriv,32);return -1;}
        uint8_t rb[FRAME_MAX_WIRE];if(recv_all(fd,rb,FRAME_HEADER_LEN+32+AEGIS_TAG_LEN)!=0){secure_memzero(opriv,32);return -1;}
        uint8_t rt,rf;size_t rl;
        if(frame_parse(rb,FRAME_HEADER_LEN+32+AEGIS_TAG_LEN,&rt,&rf,ppub,&rl,*nc,keys->dec_key)!=0){secure_memzero(opriv,32);return -1;}
        (*nc)++;
    }else{
        uint8_t rb[FRAME_MAX_WIRE];if(recv_all(fd,rb,FRAME_HEADER_LEN+32+AEGIS_TAG_LEN)!=0){secure_memzero(opriv,32);return -1;}
        uint8_t rt,rf;size_t rl;
    ...
    if(ecdh_derive(sh,opriv,ppub)!=0){secure_memzero(opriv,32);return -1;}secure_memzero(opriv,32);
    ...
    uint8_t h[32],ek[16],dk[16];H(h,b,bl);free(b);memcpy(ek,h,16);memcpy(dk,h+16,16);secure_memzero(h,32);secure_memzero(sh,32);
    if(init){memcpy(keys->enc_key,ek,16);memcpy(keys->dec_key,dk,16);}
    else    {memcpy(keys->enc_key,dk,16);memcpy(keys->dec_key,ek,16);}
    secure_memzero(ek,16);secure_memzero(dk,16);secure_memzero(nc2,16);secure_memzero(ns,16);*nc=0;return 0;
}
...
static int asym_hdr(int fd, uint8_t epk[32]) {
    uint8_t w[ALEN]; if(recv_all(fd,w,ALEN)!=0)return -1;
    memcpy(epk,w+4,32); return 0;
}
static int asym_ts(int fd, const uint8_t epk[32], const uint8_t key[16], int64_t *ts) {
    ...
    if(aegis_decrypt(tsb,ct,8,ad,ALEN,n,key,ct+8)!=0)return -1;
...
static void asym_init_key(uint8_t key[16], const uint8_t ek[32], const uint8_t sk[32], const uint8_t pk[32]) {
    ...
    uint8_t b[68];memcpy(b,ee,32);memcpy(b+32,es,32);memcpy(b+64,"init",4);uint8_t h[32];H(h,b,68);
...
static void asym_shared(uint8_t sh[32], const uint8_t ee[32], const uint8_t es[32], const uint8_t se[32]) {
    uint8_t b[102];memcpy(b,ee,32);memcpy(b+32,es,32);memcpy(b+64,se,32);memcpy(b+96,"shared",6);H(sh,b,102);
}
static void asym_resp(uint8_t k[16], const uint8_t sh[32]) { uint8_t b[36];memcpy(b,sh,32);memcpy(b+32,"resp",4);uint8_t h[32];H(h,b,36);memcpy(k,h,16);secure_memzero(h,32); }
static void asym_sess(session_keys_t *k, const uint8_t sh[32], const uint8_t ei[32], const uint8_t er[32]) {
    uint8_t b[103];memcpy(b,sh,32);memcpy(b+32,ei,32);memcpy(b+64,er,32);memcpy(b+96,"session",7);uint8_t h[32];H(h,b,103);
    memcpy(k->enc_key,h,16);memcpy(k->dec_key,h+16,16);secure_memzero(h,32);
}

/* ≡≡≡ Client: ephemeral ek→epk, static sk_c=our_priv, static pk_s=peer_pub ≡≡≡ */
int handshake_asymmetric_client(int fd, const uint8_t our_priv[32], const uint8_t peer_pub[32], int to, session_keys_t *keys) {
    uint8_t ek[32],epk[32],repk[32],ik[16],rk[16],sh[32];int64_t rts;set_to(fd,to);
    /* Step 1 */ if(ecdh_keygen(epk,ek)!=0)return -1;
    /* Step 2 */ asym_init_key(ik,ek,our_priv,peer_pub);
    /* Step 3 */ int64_t ts=timestamp_now();
                 if(asym_send(fd,epk,ts,ik)!=0){secure_memzero(ek,32);return -1;}
    /* Step 4 */ if(asym_hdr(fd,repk)!=0){secure_memzero(ek,32);return -1;}
    /* Step 5 */ {uint8_t ee[32],es[32],se[32];
                  ecdh_derive(ee,ek,peer_pub);ecdh_derive(es,our_priv,peer_pub);ecdh_derive(se,our_priv,repk);
                  asym_shared(sh,ee,es,se);secure_memzero(ee,32);secure_memzero(es,32);secure_memzero(se,32);}
                 secure_memzero(ek,32);
    /* Step 6 */ asym_resp(rk,sh);
                 if(asym_ts(fd,repk,rk,&rts)!=0){secure_memzero(sh,32);return -1;}
                 if(check_ts(rts)!=0){secure_memzero(sh,32);return -1;}
    /* Step 7 */ asym_sess(keys,sh,epk,repk);
    secure_memzero(sh,32);secure_memzero(ik,16);secure_memzero(rk,16);secure_memzero(epk,32);secure_memzero(repk,32);
    return 0;
}

/* ≡≡≡ Server: ephemeral ek→epk, static sk_s=our_priv, static pk_c=peer_pub ≡≡≡ */
int handshake_asymmetric_server(int fd, const uint8_t our_priv[32], const uint8_t peer_pub[32], int to, session_keys_t *keys) {
    uint8_t ek[32],epk[32],iepk[32],ik[16],rk[16],sh[32];int64_t its;set_to(fd,to);
    /* Step 1 */ if(asym_hdr(fd,iepk)!=0)return -1;
    /* Step 2 */ {uint8_t ee[32],es[32];ecdh_derive(ee,our_priv,iepk);ecdh_derive(es,our_priv,peer_pub);
                  uint8_t b[68];memcpy(b,ee,32);memcpy(b+32,es,32);memcpy(b+64,"init",4);uint8_t h[32];H(h,b,68);memcpy(ik,h,16);}
    /* Step 3 */ if(asym_ts(fd,iepk,ik,&its)!=0){return -1;}
                 if(check_ts(its)!=0){return -1;}
    /* Step 4 */ if(ecdh_keygen(epk,ek)!=0){return -1;}
    /* Step 5 */ {uint8_t ee[32],es[32],se[32];
                  ecdh_derive(ee,our_priv,iepk);ecdh_derive(es,our_priv,peer_pub);ecdh_derive(se,ek,peer_pub);
                  asym_shared(sh,ee,es,se);secure_memzero(ee,32);secure_memzero(es,32);secure_memzero(se,32);}
    /* Step 6 */ asym_sess(keys,sh,iepk,epk);
                 {uint8_t t[16];memcpy(t,keys->enc_key,16);memcpy(keys->enc_key,keys->dec_key,16);memcpy(keys->dec_key,t,16);secure_memzero(t,16);}
                 asym_resp(rk,sh);
    /* Step 7 */ int64_t ts=timestamp_now();int ok=asym_send(fd,epk,ts,rk);
    secure_memzero(ek,32);secure_memzero(epk,32);secure_memzero(sh,32);secure_memzero(ik,16);secure_memzero(rk,16);secure_memzero(iepk,32);
    return (ok==0&&ts>=0)?0:-1;
}
